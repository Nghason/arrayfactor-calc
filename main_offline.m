...
for iter = 1:n_iters 
    ...
    for mm = 1:length(mod_names)
        ...
        switch mod_name
            ...
            % Viet: Thêm CDS-CCT, CDS_LP 
                case 'cds_cct'
                    temp = algorithms.offline.cds_cct(fabric, coflows); 
                    temp.nac = size(temp.accepts,2);
                    temp.method = 'cds_cct';
                    out{mm} = temp;    
                case 'cds_cct_SM'
                    temp = algorithms.offline.cds_cct_SM(fabric, coflows); 
                    temp.nac = size(temp.accepts,2);
                    temp.method = 'cds_cct_SM';
                    out{mm} = temp;    
                case 'cds_lp'
                    temp = algorithms.offline.cds_lp(fabric, coflows); 
                    temp.nac = size(temp.accepts,2);
                    temp.method = 'cds_lp';
                    out{mm} = temp;                
        end
