...
for c = coflows
    ...
    clear data_cumsum_var;
    for i = 1:n_slots
        if i <= n_slots_of_coflow(cid)
        if i == 1  
            data_cumsum_var = slot_duration(1,i)*sum(C3(:,i));
        else
            data_cumsum_var = data_cumsum_var + slot_duration(1,i)*sum(C3(:,i));
        end
        end
        A =data_cumsum_var/c_vol_value;
        constraints = [constraints, cct_var(cid,i) <=  data_cumsum_var/c_vol_value];
    end
    C3 = sdpvar;
end
%%
