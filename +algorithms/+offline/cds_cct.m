% clear;
% load"data_m10_c20";
function outputs = cds_cct(fabric,coflows)
...
z_Xj = value(C3_1);
epsilon = 1e-6;
cct_avg3 = zeros(2,n_coflows+1);
for n = 1:n_coflows
    one = find(abs(z_Xj(n,:) - 1) < epsilon);
    if isempty(one)
    continue;
    else
        cct_avg3(1,n) = one(1);
        cct_avg3(2,n) = time_instants(one(1)+1);
    end
end
%calc cct at the end
for i = 1:n_coflows
...
cnt = 0;
for i = 1:size(cct_avg3,2)
    if cct_avg3(1,i) ~= 0
        cnt = cnt + 1;
    end
end
cct_avg3(2,n_coflows+1) = sum(cct_avg3(2,1:n_coflows))/cnt;
cct_avg3(2,n_coflows+1);
% Output
Zn = Zn';
    outputs.zn = Zn;
    outputs.accepts = find(Zn >= 0.99); % id of accepted coflows
    outputs.rejects = find(Zn < 0.1);   % id of rejected coflows   
    outputs.cct = cct_avg3(:, 1:n_coflows);
    outputs.ccts = cct_avg3(2,find(cct_avg3(2, 1:n_coflows) > 0));
    outputs.avg_cct = cct_avg3(2, n_coflows+1);
end
