% Implementation of [Tseng2019]
% [0,T] is divided into M disjoint time slots such that their endpoints are either a_j or d_j if not 0 or T
% Remarks:
%   slot_duration is not fixed

function OptimOut = weighted_optim_cds(OptimIn, fabric, coflows, zn_type)
...
%         zn(ci) = coflows(ci).flows(fi).volume;
%     
%         % bijm: -slot_duration
%         bijm = bijm_cell;
%         for mi = 1:n_slots_of_coflow(ci)
%             slot_id = c.addParam.slot_id(mi);
%             bijm{ci}{fi}(mi) = -slot_duration(slot_id);
%         end
%         
...
% OptimOut.output = output;
OptimOut.n_slots = OptimIn.n_slots;
OptimOut.x = x;
OptimOut.fval = fval;

% flag indicating if at least one coflow is accepted
OptimOut.isAllocSuccess = false; 
...
if exitflag > 0 %isempty(x) == 0
    ...
    if sum(zn) > 0 
        OptimOut.isAllocSuccess = true;
    end
    OptimOut.zn = (zn >= 0.99);
    OptimOut.accepts = find(zn >= 0.99); % id of accepted coflows
    OptimOut.rejects = find(zn < 0.1);   % id of rejected coflows
    OptimOut.bijm = bijm;
    OptimOut.bijm_synthesis = bijm_synthesis;
    OptimOut.nac = length(OptimOut.accepts); % for safety, instead of sum(zn)
    
    OptimOut.cct_vect = cct_vect;
    OptimOut.cct_slot = cct_slot;
    OptimOut.ccts = ccts(OptimOut.accepts); % only keep ccts of accepted coflows
